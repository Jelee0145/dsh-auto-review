...
function pairClaim(trace, claim, fail) {
    ...
    for (const [id, asked] of trace.askedById) {
        if (keyOf(asked.toolName, asked.callId) !== key)
            continue;
        if (trace.claimedById.has(id))
            fail(`ask ${JSON.stringify(key)} claimed more than once`);
...
function validateEvent(trace, event, fail) {
    if (event.type === 'turn/end') {
        ...
        if (hasUnresolvedClaims(trace))
            fail('claimed asks have no approval/decided before turn/end');
        return undefined;
    }
    if (event.type === 'approval/asked') {
        if (trace.askedById.has(event.data.id))
            fail(`approval/asked repeated id ${JSON.stringify(event.data.id)}`);
        const asked = {
...
function seedTrace(trace, session, fail) {
    ...
    for (const record of [...trace.unmatchedClaims])
...
const install = Object.assign((ctx, fail) => {
    ...
    const seed = (session) => {
        const trace = { askedById: new Map(), claimedById: new Map(), unmatchedClaims: new Set() };
